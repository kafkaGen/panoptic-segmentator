...
model = dict(
    ...
    init_cfg=None,
    panoptic_fusion_head=dict(init_cfg=None, loss_panoptic=None, num_stuff_classes=53, num_things_classes=80, type="MaskFormerFusionHead"),
    panoptic_head=dict(
        ...
        loss_dice=dict(activate=True, eps=1.0, loss_weight=5.0, naive_dice=True, reduction="mean", type="DiceLoss", use_sigmoid=True),
        ...
        pixel_decoder=dict(
            ...
            encoder=dict(
                layer_cfg=dict(
                    ffn_cfg=dict(
                        act_cfg=dict(inplace=True, type="ReLU"), embed_dims=256, feedforward_channels=1024, ffn_drop=0.0, num_fcs=2
                    ),
                    self_attn_cfg=dict(batch_first=True, dropout=0.0, embed_dims=256, num_heads=8, num_levels=3, num_points=4),
                ),
                num_layers=6,
            ),
        ...
        transformer_decoder=dict(
            ...
            layer_cfg=dict(
                ...
                ffn_cfg=dict(act_cfg=dict(inplace=True, type="ReLU"), embed_dims=256, feedforward_channels=2048, ffn_drop=0.0, num_fcs=2),
    ...
    test_cfg=dict(filter_low_score=True, instance_on=True, iou_thr=0.8, max_per_image=100, panoptic_on=True, semantic_on=False),
    train_cfg=dict(
        assigner=dict(
            match_costs=[
